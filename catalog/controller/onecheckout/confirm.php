...
class ControllerOneCheckoutConfirm extends Controller { 
		...
		if (!$json) {
			...
			foreach ($this->cart->getProducts() as $product) {
				...
				if($version_int < 1513 && $version_int >= 1500){
					...
					$version_tax = $this->tax->getTax($product['total'], $product['tax_class_id']);
				}
				$this->data['products'][] = array(
	...
	public function createorder(){
		...
		if (!$json) {
			...
			$data['payment_company_id'] = 0; //$payment_address['company_id'];	
			
			$data['payment_tax_id'] = 0; //$payment_address['tax_id'];	
