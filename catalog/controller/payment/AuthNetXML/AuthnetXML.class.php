...
class AuthnetXML
{
	...
	public function __toString()
	{
        ...
        if (!empty($this->arb_response))
        {
            $dom = new DOMDocument('1.0');
            $dom->preserveWhiteSpace = false;
            $dom->formatOutput = true;
            $dom->loadXML(self::removeResponseXMLNS($this->arb_response));
	...
	private function process()
	{
        ...
        if(($this->arb_response = curl_exec($this->ch)) !== false)
        {
			$this->response_xml = new SimpleXMLElement(str_replace('xmlns="AnetApi/xml/v1/schema/AnetApiSchema.xsd"', '', $this->arb_response));
		    curl_close($this->ch);
            unset($this->ch);
            return;
		}
