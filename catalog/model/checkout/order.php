...
class ModelCheckoutOrder extends Model {	
	public function addOrder($data) {
		...
		if (isset($this->session->data['recurring']))
		{
			$this->db->query("update `order` SET recurring = 1 WHERE order_id = " . $order_id);
		}
	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			if (isset($this->session->data['gift_wrap']) && $this->session->data['gift_wrap'])
			{
				$this->db->query("UPDATE `".DB_PREFIX."order` SET gift = 1, gift_message = '".$this->db->escape($this->session->data['gift_wrap_note'])."' WHERE order_id = '".(int)$order_id."'");
			}
			...
			if ($_SERVER['HTTP_HOST'] != 'petacoffee.local') $mail->send();
	...
	private function process_recurring()
	{
		...
		else
		{
			...
			{
				$gift_voucher = $this->session->data['voucher'];
			}  
			
			//echo "Gift Voucher: ".$gift_voucher;
