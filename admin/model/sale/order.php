...
class ModelSaleOrder extends Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified, o.recurring FROM `" . DB_PREFIX . "order` o";
		
		/*if (!empty($data['filter_recurring_id'])) {
			$sql .= " JOIN order_recurring r ON r.order_id = o.order_id ";
		}*/
		...
		/*if (!empty($data['filter_recurring_id'])) {
			$sql .= " AND r.recurring_id = '" . (int)$data['filter_recurring_id'] . "'";
		}*/
	...
	public function getTotalOrders($data = array()) {
		...
		if (isset($data['filter_recurring_id']) && !is_null($data['filter_recurring_id'])) 
		{
			$sql .= " JOIN order_recurring r ON r.order_id = `order`.order_id ";
		}
	...
	{
		$order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_history` oh WHERE oh.order_id = '" . (int)$order_id . "' ORDER BY oh.date_added DESC LIMIT 1;");
		...
		if ($order_query->num_rows) 
		{
			return array(
				'order_id'                => $order_query->row['order_id'],
				'order_status_id'         => $order_query->row['order_status_id'],
				'order_comment'            => $order_query->row['comment'],
			);
		} 
		else 
		{
	...
	public function getXMLOrderProducts($order_id) 
	{
		$order_query = $this->db->query("SELECT op.*, oo.product_option_value_id as barcode, oo.name AS option_name, oo.value AS option_value FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_option` oo WHERE op.order_product_id = oo.order_product_id AND op.order_id = '" . (int)$order_id . "' GROUP BY op.order_product_id;");
	...
	public function getSaleXML($order_id)
	{
		$order_info = $this->getOrder($order_id);
		...
		if ($order_id)
		{
			...
			$highValueDiscountAmount = 0;
			$pointsDiscountAmount = 0;
			$couponDiscountAmount = 0;
			$giftCertDiscountAmount = 0;
			
			$partial_cc = sprintf("xxxx-xxxx-xxxx-%04s",$order_info['partial_cc']);
			
			//print_r($totals);
						
			foreach($totals as $total)
			{
				if ($total['title'] == $order_info['shipping_method'])
				{
					$shipping_fee = $total['value'];
				}
				
				if ($total['title'] == 'Recurring Discount')
				{
					$reorder_amount = $total['value'];
					$orderDiscountAmount += $total['value'];
				}
				...
				{
					$couponDiscountAmount += $total['value'];
					$orderDiscountAmount += $total['value'];
				}
				
				if ($total['code'] == 'reward')
				{
					$pointsDiscountAmount += $total['value'];
					$orderDiscountAmount += $total['value'];
				}
				
				if ($total['code'] == 'voucher')
				{
					$giftCertDiscountAmount += $total['value'];
					$orderDiscountAmount += $total['value'];
				}
			}
			
			list($code, ) = explode('.', $order_info['shipping_code']);
			
			switch ($code)
			{
				case 'flat': $shipping_provider = 'Flat Rate Shipping'; break;
				case 'usps': $shipping_provider = 'USPS'; break;
			}
			...
			$output .=  '<UserId>'.$order_info['customer_id'].'</UserId>' . "\n";
			$output .=  '<FirstName>'.$order_info['firstname'].'</FirstName>' . "\n";
			$output .=  '<LastName>'.$order_info['lastname'].'</LastName>' . "\n";
			$output .=  '<Email>'.$order_info['email'].'</Email>' . "\n";
			$output .=  '<EndUserType>'.$order_info['customer_group_id'].'</EndUserType>' . "\n";			
			$output .=  '<OrderDetails>' . "\n";			
			$output .=  '	<OrderId>PE'.$order_info['order_id'].'</OrderId>' . "\n";			
			$output .=  '	<OrderDate>'.$order_info['date_added'].'</OrderDate>' . "\n";			
			$output .=  '	<OrderTotal>'.(number_format($order_info['total'],2,'.',',')).'</OrderTotal>' . "\n";			
			$output .=  '	<OrderDiscountAmt>'.(number_format($orderDiscountAmount,2,'.',',')).'</OrderDiscountAmt>' . "\n";			
			$output .=  '	<HighValueDiscountAmt>'.(number_format($highValueDiscountAmount,2,'.',',')).'</HighValueDiscountAmt>' . "\n";			
			$output .=  '	<PointsDiscountAmt>'.(number_format($pointsDiscountAmount,2,'.',',')).'</PointsDiscountAmt>' . "\n";			
			$output .=  '	<CouponDiscountAmt>'.(number_format($couponDiscountAmount,2,'.',',')).'</CouponDiscountAmt>' . "\n";			
			$output .=  '	<GiftCertDiscountAmt>'.(number_format($giftCertDiscountAmount,2,'.',',')).'</GiftCertDiscountAmt>' . "\n";			
			$output .=  '	<RecOrderDiscountAmt>'.(number_format($reorder_amount,2,'.',',')).'</RecOrderDiscountAmt>' . "\n";			
			$output .=  '	<OrderTaxAmt>'.(number_format($orderTaxAmount,2,'.',',')).'</OrderTaxAmt>' . "\n";			
			...
			foreach($products as $product)
			{
				...
				if ($options)
				{
					...
					{
						if ($option['name'] == 'Whole Bean or Ground')
						{
							$itemGround = $option['value'];
						}
					}
				}
				
				$output .=  '		<OrderItem>' . "\n";			
				$output .=  '			<ProductItemId>'.$product['order_product_id'].'</ProductItemId>' . "\n";			
				$output .=  '			<ProductItemName>'.$product['name'].'</ProductItemName>' . "\n";			
				$output .=  '			<ProductItemPrice>'.$product['price'].'</ProductItemPrice>' . "\n";			
				$output .=  '			<ProductItemAmtDiscounted>'.$orderItemAmtDiscounted.'</ProductItemAmtDiscounted>' . "\n";			
				$output .=  '           <ProductItemQuantity>'.$product['quantity'].'</ProductItemQuantity>' . "\n";			
				$output .=  '           <ProductItemGround>'.$itemGround.'</ProductItemGround>' . "\n";			
				$output .=  '           <ProductItemSku>'.$product['model'].'</ProductItemSku>' . "\n";			
				$output .=  '       </OrderItem>' . "\n";			
			}
			...
			$output .=  '			<PaymentType>'.$order_info['payment_method'].'</PaymentType>' . "\n";			
			$output .=  '           <PaymentAmount>'.number_format($order_info['total'],2,'.',',').'</PaymentAmount>' . "\n";			
			$output .=  '           <PaymentStatus>'.$order_status_query->row['name'].'</PaymentStatus>' . "\n";			
			$output .=  '			<PartialCCNumber>'.$partial_cc.'</PartialCCNumber>' . "\n";			
			$output .=  '       </PaymentMethod>' . "\n";			
			$output .=  '  	</PaymentMethods>' . "\n";			
			$output .=  '	<ShippingMethod>' . "\n";			
			$output .=  '		<ShippingProvider>'.$shipping_provider.'</ShippingProvider>' . "\n";			
			$output .=  '		<ShippingMethodName>'.str_replace('&lt;sup&gt;&#8482;&lt;/sup&gt;','',$order_info['shipping_method']).'</ShippingMethodName>' . "\n";			
			$output .=  '	    <ShippingFee>'.$shipping_fee.'</ShippingFee>' . "\n";			
			$output .=  '	</ShippingMethod>' . "\n";			
			$output .=  '   <GiftMessage>'.$order_info['gift_message'].'</GiftMessage>' . "\n";			
			$output .=  '   <Gift>'.$order_info['gift'].'</Gift>' . "\n";			
			$output .=  '</OrderDetails>' . "\n";
			$output .=  '<ShippingAddresses>' . "\n";
			$output .=  '   <Address>' . "\n";
			$output .=  '		<FirstName>'.$order_info['shipping_firstname'].'</FirstName>' . "\n";
			$output .=  '		<LastName>'.$order_info['shipping_lastname'].'</LastName>' . "\n";
			$output .=  '		<StreetAddress1>'.$order_info['shipping_address_1'].'</StreetAddress1>' . "\n";
			$output .=  '		<StreetAddress2>'.$order_info['shipping_address_1'].'</StreetAddress2>' . "\n";
			$output .=  '		<City>'.$order_info['shipping_city'].'</City>' . "\n";
			$output .=  '		<State>'.$order_info['shipping_zone_code'].'</State>' . "\n";
			$output .=  '		<Country>'.$order_info['shipping_iso_code_2'].'</Country>' . "\n";
			$output .=  '		<PostalCode>'.$order_info['shipping_postcode'].'</PostalCode>' . "\n";
			$output .=  '		<PhoneNumber>'.$order_info['telephone'].'</PhoneNumber>' . "\n";
			$output .=  '	</Address>' . "\n";
			$output .=  '</ShippingAddresses>' . "\n";
			$output .=  '<BillingProfile>' . "\n";
			$output .=  '	<FirstName>'.$order_info['payment_firstname'].'</FirstName>' . "\n";
			$output .=  '   <LastName>'.$order_info['payment_lastname'].'</LastName>' . "\n";
			$output .=  '   <Email>'.$order_info['email'].'</Email>' . "\n";
			$output .=  '	<Address>' . "\n";
			$output .=  '		<Name>'.$order_info['payment_firstname'].'  ' .$order_info['payment_lastname'].'</Name>' . "\n";
			$output .=  '       <StreetAddress1>'.$order_info['payment_address_1'].'</StreetAddress1>' . "\n";
			$output .=  '       <StreetAddress2>'.$order_info['payment_address_2'].'</StreetAddress2>' . "\n";
			$output .=  '       <City>'.$order_info['payment_city'].'</City>' . "\n";
			$output .=  '       <State>'.$order_info['payment_zone_code'].'</State>' . "\n";
			$output .=  '       <Country>'.$order_info['payment_iso_code_2'].'</Country>' . "\n";
			$output .=  '       <PostalCode>'.$order_info['payment_postcode'].'</PostalCode>' . "\n";
			$output .=  '       <PhoneNumber>'.$order_info['telephone'].'</PhoneNumber>' . "\n";
			$output .=  '       </Address>' . "\n";
			$output .=  '</BillingProfile>' . "\n";
			$output .=  '<Notes>'.htmlentities(strip_tags(str_replace("\n", " ", $order_info['comment']))).'</Notes>' . "\n";
		}
	...
	public function xgetSaleXML($order_id) 
	{
		$order_info = $this->getOrder($order_id);
		...
		if ($order_id) 
		{
			...
			$order = $order_info; //$this->model_checkout_order->getOrder( (int)$order_id );
			
			$order_status = $this->getOrderStatus( (int)$order_id );
			$order_products = $this->getXMLOrderProducts( (int)$order_id );
			...
			if (!empty($order_totals)) 
			{
				foreach($order_totals as $order_total ) 
				{
					...
					{
						case 'sub_total': $sub_total = $order_total; break;						
						case 'shipping': $shipping = $order_total; break;
						case 'coupon': $coupon = $order_total; break;
						case 'total': $total = $order_total; break;
					}
				}
			}

			if (strstr($order_status['order_comment'], 'Trans')) 
			{
				$trxn_id = explode(" ", $order_status['order_comment']);
				$payment_method = $trxn_id[0];
				$trxn_id = str_replace('#', '', $trxn_id[4]);
				$order_status_comment = $order_status['order_comment'];
			} 
			else 
			{
				$trxn_id = $order['order_id'];
				$payment_method = $order['payment_method'];
				$order_status_comment = 'NO COMMENT';
			}

			if ( isset($shipping['title']) ) 
			{
				$delivery_comment = trim( str_replace(array("\r\n", "\r", "\n"), ' ', $shipping['title']) );
			} 
			else 
			{
				$delivery_comment = '';
...
XML;
			...
			$tx = $sxe->addChild('TRANSACTION');


			$tx->addChild('ORDERID', $order['order_id']); 

			$tx->addChild('DATE', date('d/m/Y', strtotime($order['date_added'])));

			$tx->addChild('TIME', date('H:i:s', strtotime($order['date_added'])));

			$tx->addChild('TRANNO', $trxn_id);


			$tx->addChild('TYPE', 'ORDER');

			$tx->addChild('DELIVERYCHARGE', number_format( ( isset($shipping['value']) ? $shipping['value'] : 0 ), 2));

			$tx->addChild('DELIVERYCOMMENT', $delivery_comment);

			$tx->addChild('COMMENTS', $comment);

			$tx->addChild('TOTAL', number_format($order['total'], 2));

			$tx->addChild('PAYMENT', number_format($order['total'], 2));

			$tx->addChild('PAYMENTMETHOD', $payment_method);

			$tx->addChild('PAYMENTCOMMENT', $order_status_comment);

			$tx->addChild('ACCNO', '');

			$tx->addChild('TITLE', '');

			$tx->addChild('FIRSTNAME', $order['shipping_firstname']);

			$tx->addChild('SURNAME', $order['shipping_lastname']);

			$tx->addChild('PHONE1', $order['telephone']);

			$tx->addChild('PHONE2', $order['fax']);

			$tx->addChild('EMAIL', $order['email']);

			$tx->addChild('ADDRESS1', $order['shipping_company'] . ' ' . $order['shipping_address_1']);

			$tx->addChild('ADDRESS2', $order['shipping_address_2']);

			$tx->addChild('ADDRESS3', $order['shipping_city'] . ' ' . $order['shipping_zone_code']);

			$tx->addChild('POSTCODE', $order['shipping_postcode']);

			$tx->addChild('CUSTOMERCOMMENT', '');
			...
			if (!empty($order_products)) {
				...
				foreach($order_products as $key=>$row ) {
					...
					$items = $tx->addChild('Items');

					$items->addChild('QTY', $row['quantity']);

					$items->addChild('CODE', $row['barcode']);

					$items->addChild('DESC', $row['name']);

					$items->addChild('PRICE', number_format($row['price'], 2));

					$items->addChild('DISC', '');

					$items->addChild('SELL', number_format($row['price'], 2));

					$items->addChild('NETT', number_format(($row['price'] * $row['quantity']), 2));

					$items->addChild('COMMENT', '');
			...
			if (isset($coupon)) {
				...
				$items->addChild('QTY', 1);

				$items->addChild('CODE', 9999998);

				$items->addChild('DESC', $coupon['title']);

				$items->addChild('PRICE', number_format($coupon['value'], 2));

				$items->addChild('DISC', '');

				$items->addChild('SELL', 0);

				$items->addChild('NETT', number_format($coupon['value'], 2));
