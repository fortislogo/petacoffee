...
class ControllerSaleOrder extends Controller {
	private $error = array();
  	...
  	protected function getList() {
		...
		$this->data['regenerate'] = $this->url->link('sale/order/regenerate', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function regenerate()
	{
		...
		if (isset($this->request->post['selected'])) 
		{
			...
			foreach ($this->request->post['selected'] as $order_id) 
			{
				...
				$output .= $this->model_sale_order->getSaleXML($order_id);
	...
	{		
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateStatus()) 
		{
			...
			{
        		list($order_id, $tracking, $status)	= $data;
				...
				$order = array('order_status_id' => $order_status_id, 'comment' => $tracking , 'notify' => false);
				$this->model_sale_order->addOrderHistory($order_id, $order);
    		}
			
			$this->redirect($this->url->link('sale/order', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		
		$this->language->load('sale/order_status');
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
       		'text'      => $this->language->get('heading_title'),
			'href'      => $this->url->link('sale/order/status', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => ' :: '
   		);
		...
		$this->template = 'sale/order_status.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validateStatus()
	{
		...
		if (!isset($name[1]) || $name[1] != 'csv')
		{
			$this->error['warning'] = 'Invalid file. Upload the correct file.';
		}
